...
export default function DanhGiaPage() {
  ...
  const loadReviews = useCallback(async () => {
    try {
      ...
      const response = await authorizedFetch(`${API_BASE}/api/admin/reviews?${query.toString()}`, {
        cache: "no-store",
      });
  ...
  const handleDelete = async (entity: ReviewEntity, id: number) => {
    ...
    try {
      setDeletingId(id);
      const response = await authorizedFetch(`${API_BASE}/api/admin/reviews/${entity}/${id}`, {
        method: "DELETE",
      });
      const data = await response.json();
